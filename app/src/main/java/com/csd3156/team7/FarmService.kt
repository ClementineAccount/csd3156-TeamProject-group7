...
public class FarmService : Service() {
    ...
    private var cubeGrowthRate : Int = 5
            ...
            override fun run() {
                //UPDATE PRYAMID
                    GlobalScope.launch(Dispatchers.IO)
                    {
                        ...
                        if (farmRepository.currentFarmShape == "Pyramid")
                        {
                        ...
                        else if (farmRepository.currentFarmShape == "Cube")
                        {
                            var cubeItem = shopRepository.getItemByName("Cube")

                            //TODO: Increment it here
                            //TODO: Increment by formula that takes into account the farm
                            //TODO: Set it back
                            var updateCount = shopRepository.getItemQuantity(cubeItem.itemId)
                            updateCount += cubeGrowthRate

                            Log.d("FarmService", "cubeItem Quantity: ${updateCount}")
                            shopRepository.updateQuantity(cubeItem.itemId, updateCount)
                            cubeItem = shopRepository.getItemByName("Cube")
                        }
                    }
